...
def train(args):
    ...
    def collate_fn(batch):
        crop_size = 60 # latent hw size 60*60 -> video size 480 *480
        crop_frames = 9 # latent T size 9
    ...
    def train_dataloader(rank):
        ...
        image_paths = sorted(glob(os.path.join('./dataset/image_pth', '**', '*.pth'), recursive=True))
        video_paths = sorted(glob(os.path.join('./dataset/pth_file', '**', '*.pth'), recursive=True))
        ...
        sampler = MultitypeDistributedBatchSampler(
            ...
            data_prob=[0.0, 1.0], #paper use 0.1, 0.9
