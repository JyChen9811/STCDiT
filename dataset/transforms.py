...
def single_random_crop(img_gts, gt_patch_size, gt_path=None):
    ...
    if h_gt < gt_patch_size[0] or w_gt < gt_patch_size[1]:
...
def single_random_resize_crop(img_gts, gt_patch_size, gt_path=None):
    """Random crop. Support Numpy array and Tensor inputs.
    ...
    target_w = int(scale * 1920)
    target_h = int(scale * 1080)
    ...
    # crop corresponding gt patch
    if input_type == 'Tensor':
        img_gts = [v[:, :, start_y:start_y + target_h, start_x:start_x + target_w] for v in img_gts]
    else:
        img_gts = [v[start_y:start_y + target_h, start_x:start_x + target_w, ...] for v in img_gts]
...
def single_random_resize(img_gts, gt_patch_size, gt_path=None):
    """Random crop. Support Numpy array and Tensor inputs.
...
def single_specific_crop(img_gts, gt_patch_size, gt_path=None):
    """Random crop. Support Numpy array and Tensor inputs.
    ...
    # randomly choose top and left coordinates for lq patch
    top = (h_gt - gt_patch_size[0]) // 2
    left = w_gt - gt_patch_size[1] - 1
