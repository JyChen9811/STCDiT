...
def parse_args():
    ...
    parser.add_argument("--skip_existing", action="store_true",
                        help="Skip output video if it already exists.")
...
def try_claim(claim_dir: str, name: str) -> str | None:
    """Atomically claim a video. Returns lock path on success, None if already claimed."""
    os.makedirs(claim_dir, exist_ok=True)
    lock_path = os.path.join(claim_dir, name + ".lock")
    try:
...
claim_dir = os.path.join(save_path, ".claims")

# LPT scheduling: process larger files first to reduce tail latency.
all_pairs = sorted(
    zip(video_files, caption_files),
    key=lambda p: os.path.getsize(p[0]),
    reverse=True,
)

pid = os.getpid()
for video_file_path, caption_file_path in all_pairs:
    ...
    lock_path = try_claim(claim_dir, name)
    if lock_path is None:
        continue  # already claimed by another process

    print(f"[pid {pid}] Processing: {video_file_path}, {caption_file_path}")
    try:
        ...
        save_video(video, out_path, fps=24, quality=9, target_size=target_size)
    except Exception as e:
        # Release the lock so another process can retry this video.
        if not os.path.exists(out_path) and os.path.exists(lock_path):
            try:
                os.remove(lock_path)
            except OSError:
                pass
        print(f"[pid {pid}] Failed {name}: {e}")
